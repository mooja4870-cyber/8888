...
class AutoTrader:
    ...
    def sync_active_positions(self, actual_positions: List[Dict]):
        ...
        for p in actual_positions:
            ...
            if sym not in self.position_strategies:
                ...
                self.position_partial_done[sym] = True  # 메타 없는 기존 포지션은 분할익절 스킵(안전)
                removed_any = True
    ...
    async def on_signal(self, sig: Signal):
        ...
        base_bluefrog = getattr(self.cfg, "USE_BLUEFROG", False)
        ...
        async with self.lock:
            ...
            # ── [Tier A ②] 손익비 하한 게이트 ──
            # TP가 고정폭인 구조에서 RR = TP폭/SL폭이다. 손절폭이 넓은 신호일수록
            # 손익비가 나쁜데 종전에는 사이즈가 같아 손실만 커졌다.
            # 실측 190건: |손실| ≥2% 30건이 전체 손실의 57%(-7.84 USDT).
            ...
            # ── [Tier A ①] 리스크 균등화 사이징 ──
            # 증거금을 손절폭에 반비례시켜 거래당 USDT 리스크를 일정하게 맞춘다.
            # 총 노출 수준은 유지하고(기준 손절폭에서 배수 1.0) 분산만 줄인다.
            if bool(getattr(self.cfg, "USE_RISK_NORMALIZED_SIZING", False)) and dynamic_sl_pct > 0:
                ...
                # 정규화 후에도 최소 증거금을 넘어야 한다. 못 넘으면 '올바른 크기로
                # 실을 수 없는 거래'이므로 억지로 키우지 않고 포기한다.
            ...
            if result:
                ...
                csv_log({
                    ...
                    "trade_mode": self.position_trade_modes.get(sig.symbol, "역방향" if getattr(self.cfg, "USE_BLUEFROG", False) else "순방향"),
                })
    ...
    async def _risk_check(self, sig: Signal) -> tuple[bool, str]:
        ...
        if sig.strategy_type in ("DoubleBottom", "DoubleTop"):
            ...
            # 신호 수식 5조건 + 두 라인 돌파가 이미 필터다. 별도 강도 기준을 두면
            # 문서 원칙(조건 충족 시 즉시 매수)과 어긋난다. 8409와 동일 처리.
            required_strength = 0
        else:
