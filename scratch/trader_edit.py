...
class AutoTrader:
    ...
    def register_trade_result(self, symbol: str, pnl: float):
        ...
        if max_consec > 0:
            ...
            self.save_cooldowns()   # [2026-09-04] 카운터 변경 즉시 저장
        ...
        if loss_n <= 0 or hours <= 0:
            return
        if pnl < 0:
            # [1] ENABLE_STOP_AND_REVERSE
            if getattr(self.cfg, "ENABLE_STOP_AND_REVERSE", False) and symbol in self._closed_cache:
                info = self._closed_cache[symbol]
                duration_hrs = (datetime.now().timestamp() - info["ts"]) / 3600.0
                if duration_hrs < 2.0:
                    logger.warning(f"🔄 [STOP & REVERSE] {symbol} 휩쏘 손절 감지(지속시간 {duration_hrs:.1f}h). 역방향 2배수 진입 트리거!")
                    from core.strategy import Signal
                    rev_side = "short" if info["side"] == "long" else "long"
                    # Create fake signal
                    sig = Signal(
                        symbol=symbol,
                        direction=rev_side,
                        strength=100, ema_ok=True, bb_ok=True, macd_ok=True,
                        close=0.0, ema200=0.0, bb_upper=0.0, bb_lower=0.0, rsi=0.0,
                        reason="Whipsaw Stop & Reverse",
                        strategy_type="TSMOM",
                        sl_price=0.0, tp_price=0.0, atr=0.0
                    )
                    setattr(sig, "margin_scale", 2.0) # 2배수
                    import asyncio
    ...
    async def guard_position_protection(self, positions: Optional[List[Dict]] = None) -> int:
        ...
        **둘 다 없는 '고아'** 상태가 된다. 이때 최대보유시간·트레일링·분할익절·손절이
        ...
        try:
            await self._monitor_advanced_strategies(positions)
        except Exception as e:
            logger.error(f"[_monitor_advanced_strategies] 에러: {e}")
    ...
    async def _monitor_advanced_strategies(self, positions: List[Dict]):
        if not positions:
            return
        ...
        for p in positions:
            sym = p.get("symbol")
            try:
                entry = float(p.get("entry_price") or p.get("entryPrice") or 0.0)
                mark = float(p.get("mark_price") or p.get("markPrice") or 0.0)
                amt = float(p.get("position_amount") or p.get("positionAmt") or 0.0)
            except:
                continue
            ...
            state = self.position_advanced_state.get(sym, {})
            if not state:
                continue

            # [2] ENABLE_PYRAMIDING (진입 물량 분할)
            if getattr(self.cfg, "ENABLE_PYRAMIDING", False) and not state.get("pyramid_done", True):
                if pnl_pct >= 0.005: # 예: 0.5% 수익 시 불타기 (목표가의 30% 지점 등)
                    target_margin = state.get("pyramid_target_margin", 0)
                    if target_margin > 0:
                        logger.info(f"🧱 [PYRAMIDING] {sym} 추세 확정 (+{pnl_pct*100:.2f}%). 70% 잔여 물량 진입: ${target_margin:.2f}")
                        state["pyramid_done"] = True
                        self.save_active_positions()
                        try:
                            # 시장가 불타기 주문
                            await self.client.create_order(sym, "buy" if side == "long" else "sell", "market", margin_usdt=target_margin, leverage=self.cfg.LEVERAGE)
                        except Exception as e:
                            logger.error(f"[PYRAMIDING ERROR] {e}")

            # [3] ENABLE_DYNAMIC_PARTIAL_TP (부분 익절 & 본절)
            if getattr(self.cfg, "ENABLE_DYNAMIC_PARTIAL_TP", False) and not state.get("partial_tp_done", False):
                if pnl_pct >= 0.015: # +1.5% 도달 시
                    logger.info(f"💰 [PARTIAL TP] {sym} +1.5% 수익 도달. 50% 분할 익절 및 본절 이동 실행!")
                    state["partial_tp_done"] = True
                    self.save_active_positions()
                    try:
                        # 절반 시장가 청산
                        close_amt = abs(amt) * 0.5
                        close_side = "sell" if side == "long" else "buy"
                        await self.client.create_order(sym, close_side, "market", quantity=close_amt, reduce_only=True)
                        # SL 본절 이동
                        rearm = getattr(self.client, "rearm_position_protection", None)
                        if rearm:
                            await rearm(sym, side, entry, sl_price=entry)
                    except Exception as e:
                        logger.error(f"[PARTIAL TP ERROR] {e}")

            # [6] ENABLE_CHANDELIER_TRAILING_STOP (트레일링 스탑)
            if getattr(self.cfg, "ENABLE_CHANDELIER_TRAILING_STOP", False):
                high = state.get("highest_price", 0.0)
                low = state.get("lowest_price", float('inf'))
                if side == "long" and mark > high:
                    state["highest_price"] = mark
                elif side == "short" and mark < low:
                    state["lowest_price"] = mark
                
                # 수익 2% 이상일 때만 작동
                if pnl_pct >= 0.02:
                    trail_dist = 0.01 # -1% 꺾이면 청산
                    if side == "long":
                        trail_sl = state["highest_price"] * (1 - trail_dist)
                        if mark <= trail_sl:
                            logger.info(f"🏃 [TRAILING STOP] {sym} 고점 대비 -1% 꺾임. 전량 시장가 청산! (고점:{state['highest_price']:.4f})")
                            await self.client.create_order(sym, "sell", "market", quantity=abs(amt), reduce_only=True)
                    else:
                        trail_sl = state["lowest_price"] * (1 + trail_dist)
                        if mark >= trail_sl:
                            logger.info(f"🏃 [TRAILING STOP] {sym} 저점 대비 +1% 반등. 전량 시장가 청산! (저점:{state['lowest_price']:.4f})")
                            await self.client.create_order(sym, "buy", "market", quantity=abs(amt), reduce_only=True)
    ...
    def sync_active_positions(self, actual_positions: List[Dict]):
        ...
        for sym in list(self.position_strategies.keys()):
            if sym not in actual_symbols:
                ...
                # [1. Whipsaw Stop & Reverse] 정보 저장
                if sym in self.position_advanced_state:
                    self._closed_cache[sym] = {
                        "ts": self.position_advanced_state[sym].get("ts", 0),
                        "side": self.position_advanced_state[sym].get("side", "long")
                    }
                self.recently_entered.pop(sym, None)
                self.position_strategies.pop(sym, None)
                self.position_advanced_state.pop(sym, None)
    ...
    def save_cooldowns(self):
        try:
            ...
            # 워치독 자동 재기동·수동 재기동이 잦아, MAX_CONSEC_SL_PER_DAY를 걸어두고도
            # 연패가 그대로 진행됐다(8410 실측: 9연패). 정지는 '당일' 단위이므로
            # 날짜와 함께 저장해야 다음날 정상 해제된다.
            data["daily_consec_sl"] = int(getattr(self, "_daily_consec_sl", 0))
            data["consec_sl_date"] = self._consec_sl_date.isoformat() if getattr(self, "_consec_sl_date", None) else None
            data["halted_by_consec_sl"] = bool(getattr(self, "_halted_by_consec_sl", False))
            data["symbol_loss_streak"] = {k: int(v) for k, v in (getattr(self, "symbol_loss_streak", {}) or {}).items() if v}
    ...
    async def on_signal(self, sig: Signal):
        ...
        async with self.lock:
            ...
            if risk_pct > 0 and not getattr(self.cfg, "USE_AUTO_COMPOUND", False):
                try:
                    balance = await self.client.get_balance()
                    total_bal = float(balance.get("total", 0.0) or 0.0)
                    if total_bal > 0:
                        ...
                        calc_margin_usdt = min(calc_margin_usdt, margin_cap)
                                                ...
                                                # 바이낸스 등 최소 주문금액(Notional) 미달 방어 (5.5 USDT 기준)
                        _min_notional = float(getattr(self.cfg, "EXCHANGE_MIN_NOTIONAL_USDT", 5.0))
                        _min_margin = (_min_notional * 1.1) / lev
                        calc_margin_usdt = max(_min_margin, calc_margin_usdt)
            ...
            # --- [VOLATILITY SIZING] ---
            if getattr(self.cfg, "USE_VOLATILITY_SIZING", False):
                try:
                    vol_adx_th = float(getattr(self.cfg, "VOLATILITY_ADX_THRESHOLD", 40.0))
                    # Check if signal has ADX
                    if hasattr(sig, "adx") and sig.adx >= vol_adx_th:
                        old_m = margin_usdt
                        margin_usdt = round(margin_usdt * 0.5, 2)
                        logger.warning(f"🛡️ [VOLATILITY SIZING] ADX({sig.adx:.1f} >= {vol_adx_th}) 고변동성 감지. 시드 투입량 50% 축소: {old_m} -> {margin_usdt}")
                        send_telegram_alert(f"🛡️ *[VOLATILITY SIZING]* {sig.symbol} ADX({sig.adx:.1f}) 초과로 고변동성 감지! 시드 투입량을 50% 축소합니다. ({old_m} -> {margin_usdt})")
                except Exception as e:
                    logger.warning(f"[VOLATILITY SIZING] 에러: {e}")

            # [4] ENABLE_VOLATILITY_SCALING (ATR 기반)
            if getattr(self.cfg, "ENABLE_VOLATILITY_SCALING", False) and hasattr(sig, "volatility_ratio"):
                if getattr(sig, "volatility_ratio", 1.0) > 1.5:
                    old_m = margin_usdt
                    margin_usdt = round(margin_usdt * 0.5, 2)
                    logger.warning(f"🛡️ [VOLATILITY SIZING] ATR 급등(1.5배 초과). 시드 50% 삭감: {old_m} -> {margin_usdt}")
                    send_telegram_alert(f"🛡️ *[VOLATILITY SIZING]* {sig.symbol} ATR 급등! 시드 투입량을 50% 축소합니다. ({old_m} -> {margin_usdt})")

            # --- [DRAWDOWN MARGIN SCALING] ---
            regime_flags = getattr(sig, "regime_flags", {})
            scale = regime_flags.get("margin_scale", 1.0)
            if scale < 1.0:
                old_m = margin_usdt
                margin_usdt = round(margin_usdt * scale, 2)
                logger.warning(f"🛡️ [DRAWDOWN GUARD] 연패 감지로 인한 진입 시드 강제 축소: {old_m} -> {margin_usdt}")

            # [2] ENABLE_PYRAMIDING (진입 물량 30% 분할)
            is_pyramiding = False
            if getattr(self.cfg, "ENABLE_PYRAMIDING", False):
                old_m = margin_usdt
                margin_usdt = round(margin_usdt * 0.3, 2)
                is_pyramiding = True
                logger.info(f"🧱 [PYRAMIDING] 30% 정찰병 진입: {old_m} -> {margin_usdt}")

            # 최소 진입 증거금 하한선 1.1달러 보정
            if margin_usdt < 1.1:
                old_margin = margin_usdt
                margin_usdt = 1.1
                logger.warning(f"[INFO] 증거금 강제 하한선 보정: {old_margin:.2f} -> 1.1 USDT (최소 1달러 충족)")

            # ── SL/TP 계산 ──
            if getattr(self.cfg, "USE_ATR_SL", False) and sig.atr > 0 and sig.close > 0:
                atr_sl_mult = float(getattr(self.cfg, "ATR_SL_MULT", 1.5))
                atr_tp_mult = float(getattr(self.cfg, "ATR_TP_MULT", 3.0))
                dynamic_sl_pct = (sig.atr * atr_sl_mult) / sig.close
                dynamic_tp_pct = (sig.atr * atr_tp_mult) / sig.close
                logger.info(
                    f"[ATR DYNAMIC SL/TP] {sig.symbol} 강제 적용: "
                    f"SL={dynamic_sl_pct*100:.3f}% (ATR*{atr_sl_mult}), "
                    f"TP={dynamic_tp_pct*100:.3f}% (ATR*{atr_tp_mult})"
                )
            elif has_swing_sltp:
                ...
                if _sl_cap > 0 and dynamic_sl_pct > _sl_cap:
                    dynamic_sl_pct = _sl_cap
                    dynamic_tp_pct = dynamic_sl_pct * float(getattr(self.cfg, "RR_RATIO", 5.0))
                logger.info(
            ...
            if result:
                ...
                try: is_pyramiding
                except NameError: is_pyramiding = False
                
                self.position_advanced_state[sig.symbol] = {
                    "ts": datetime.now().timestamp(),
                    "side": side,
                    "pyramid_done": not is_pyramiding,
                    "pyramid_target_margin": (margin_usdt / 0.3 * 0.7) if is_pyramiding else 0.0,
                    "highest_price": 0.0,
    ...
    async def _risk_check(self, sig: Signal) -> tuple[bool, str]:
        ...
        try:
            ...
            if ticker:
                ...
                if bid > 0 and ask > 0:
                    ...
                    # [2026-09-11] 1%가 코드에 박혀 MAX_ENTRY_SPREAD_PCT 설정이 죽어 있었다.
                    _spd_cap = float(getattr(self.cfg, "MAX_ENTRY_SPREAD_PCT", 0.01))
                    if spread_pct > _spd_cap:
                        return False, f"진입 전 스프레드 과다 차단 (Spread: {spread_pct*100:.2f}% > 1%)"
                
                # W3. Volatility
                if high24 > 0 and low24 > 0:
                    volatility_24h = (high24 - low24) / low24
                    # [2026-09-11] 15%가 코드에 박혀 MAX_24H_VOLATILITY_PCT 설정이 죽어 있었다.
                    _vol_cap = float(getattr(self.cfg, "MAX_24H_VOLATILITY_PCT", 0.15))
                    if volatility_24h > _vol_cap:
        ...
        if sig.strategy_type in ("DoubleBottom", "DoubleTop"):
            ...
            required_strength = 80
        elif getattr(sig, "strategy_type", "") == "TSMOM":
            required_strength = 0
        ...
        if free < required_margin:
            # 예외 톨러런스: 요구 증거금과 가용 증거금 차이가 미세(예: 잔고의 5% 이내 또는 2달러 이내)하다면 차단하지 않음
            if required_margin - free < 2.0 and free > 1.1:
                logger.info(f"[TOLERANCE] 가용 증거금 미세 부족 ({free:.2f} < {required_margin:.2f}). 톨러런스 적용하여 진입 승인.")
                pass
            else:
