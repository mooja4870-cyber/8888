...
무엇을 바꿨나
─────────────
[1차] 2026-08-25 15:47 KST
  TRAILING_ACTIVATE_PCT  0.80% → 1.50%
  TRAILING_CALLBACK_PCT  0.30% → 1.20%   (core/config.py 설계 기본값으로 복원)

[2차] 2026-08-25 16:33 KST
  STOP_LOSS_PCT    0.70% → 3.00%   (실제 진입 손절 −3.2~3.8%와 정합)
  TAKE_PROFIT_PCT  1.20% → 5.00%   (실제 진입 익절 +5%·SNIPER_TP1_PCT와 일치)

  2차가 더 근본적이다. TAKE_PROFIT_PCT는 단순한 익절선이 아니라
  trailing_stop_manager.py:257의 **"익절 돌파 락인" 기준**으로 쓰인다:
      if raw_move >= tp_pct:  new_sl_price = entry * (1 + tp_pct)
  즉 +1.2%에 닿는 순간 손절선이 +1.2%에 못박혀, 이후 조금만 밀리면 거기서 끝났다.
  이익거래의 37%가 +1.0~1.6%에 몰리고 바로 아래 +0.8~1.0%는 1건뿐이었던 이유다.
  STOP_LOSS_PCT는 보호주문 재무장 시 손절폭을 −3%대에서 −0.7%로 좁혀,
  손실거래의 25%를 −0.7% 부근에서 조기 절단하고 있었다.

  ⚠️ 두 변경이 하루 안에 겹쳤다. 분리 측정은 불가능하니 묶어서 판단할 것.
...
CUTOFF = "2026-08-25 15:47:00"      # 1차 변경 시각(2차는 16:33)          # 트레일링 변경 시각
