...
def decide_action(m, diag, state):
    ...
    last = state.get(m["bot"], {}).get("last_action_ts", 0)
    if time.time() - last < ACTION_COOLDOWN_H * 3600:
    ...
    if dr is not None and dr < HALT_DAILY_RET and m["auto"]:
        return ({"AUTO_TRADING": False},
                f"일평균 {dr:+.2f}% < {HALT_DAILY_RET}% — 자동매매 긴급 정지"), None
...
# ── 리포트 ───────────────────────────────────────────────────
def build_report(m, diag, action, applied, restarted):
    L = [f"🩺 **[수익성 워치독] {m['bot']}**",
...
def main():
    ...
    for m in all_metrics:
        ...
        if action:
            ...
            state.setdefault(bot, {})["last_action_ts"] = time.time()
            state[bot]["last_action"] = action[1]
            state[bot]["at"] = now_str()
        elif why:
            log(f"[{bot}] {why}")

        post_discord(build_report(m, diag, action, applied, restarted))
