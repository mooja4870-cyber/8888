...
class AutoTrader:
    ...
    async def on_signal(self, sig: Signal):
        ...
        async with self.lock:
            ...
            # --- 🚀 [PROFIT BOOSTER SIZING SCALING] ---
            booster_mult = float(getattr(sig, "booster_sizing_mult", 1.0) or 1.0)
            if booster_mult > 1.0:
                old_m = margin_usdt
                margin_usdt = round(margin_usdt * booster_mult, 2)
                margin_source = f"{margin_source}+booster({booster_mult}x)"
                logger.info(f"🚀 [PROFIT BOOSTER SIZING] {sig.symbol} 부스터 사이징 적용 ({booster_mult}x): ${old_m:.2f} -> ${margin_usdt:.2f}")
    ...
    async def _risk_check(self, sig: Signal) -> tuple[bool, str]:
        ...
        if sig.strategy_type in ("DoubleBottom", "DoubleTop"):
            ...
            required_strength = 80
        elif getattr(sig, "strategy_type", "") == "DeepLearning":
