...
def collect_hourly_data(num_hours=50):
    now = time.time()
...
def send_report():
    ...
    timestamps, series_a, series_b, bot_series = collect_hourly_data(num_hours=50)
    ...
    msg_groups = (
        f"📢 **[8888 봇 그룹별 50시간 일평균수익률 추이 리포트]**\n"
        f"📅 **집계 시각**: `{now_str}` (최근 50시간 정시 추이)\n"
        f"--------------------------------------------------\n"
        f"{graph_a}\n\n"
        f"{graph_b}\n"
        f"--------------------------------------------------"
