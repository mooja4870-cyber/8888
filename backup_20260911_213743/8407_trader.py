...
class AutoTrader:
    ...
    def __init__(self, client: OKXClient):
        ...
        # [2026-09-05] 다단 분할익절(40/30/30) 단계 카운터.
        # position_partial_done은 불리언이라 1단 발동 후 그 종목이 영구 제외됐다.
        # 0=미발동, 1=1단 완료, 2=2단 완료, 3=완전청산. 99=메타 없는 구 포지션(건너뜀).
        self.position_scaleout_stage: Dict[str, int] = {}
        ...
        try:
            ...
            if os.path.exists("data/active_positions.json"):
                ...
                for sym, val in data.items():
                    ...
                    # [2026-09-05] 다단 분할익절 단계 복구
                    self.position_scaleout_stage[sym] = int(val.get("scaleout_stage", 0) or 0)
        ...
        try:
            ...
            for sym in list(self.position_strategies.keys()):
                data[sym] = {
                    ...
                    "scaleout_stage": int(self.position_scaleout_stage.get(sym, 0)),
                }
    ...
    async def guard_position_protection(self, positions: Optional[List[Dict]] = None) -> int:
        ...
        **둘 다 없는 '고아'** 상태가 된다. 이때 최대보유시간·트레일링·분할익절·손절이
        전부 무동작이다. 실측: 8402 ALLO 35시간17분 / 8403 PUMP 60시간 손절 부재.
    ...
    def sync_active_positions(self, actual_positions: List[Dict]):
        ...
        for sym in list(self.position_strategies.keys()):
            if sym not in actual_symbols:
                ...
                self.position_scaleout_stage.pop(sym, None)
                removed_any = True
        
        for p in actual_positions:
            sym = p["symbol"]
            if sym not in self.position_strategies:
                logger.warning(f"[PERSIST] {sym} 포지션이 거래소에 존재하나 로컬 메타데이터가 없음 → 기본값(Breakout) 부여")
                self.position_strategies[sym] = "Breakout"
                self.position_atr_activations[sym] = getattr(self.cfg, 'TRAILING_ACTIVATE_PCT', 0.015)
                self.position_atr_callbacks[sym] = getattr(self.cfg, 'TRAILING_CALLBACK_PCT', 0.012)
                self.position_exit_profiles[sym] = "SL/TP"
                # [2026-09-05 수정] 종전에는 True/99로 두어 **분할익절에서 영구 제외**했다.
                # 진입은 bot.py가 하고 분할익절은 대시보드(app.py)가 실행하는 구조라,
                # 대시보드는 bot.py가 새로 잡은 포지션을 모른 채 여기로 떨어진다.
                # 그 결과 **bot.py가 새로 진입한 포지션이 전부 분할익절 대상에서 빠졌다**
                # (실측 2026-09-05: ARB 01:31 진입 직후 partial_done=True로 기록됨).
                # 메타를 몰라도 분할익절 대상에는 넣는다 — 익절은 놓치면 손해고,
                # 잘못 걸려도 이익 구간에서 일부를 확정할 뿐이라 위험이 비대칭이다.
                self.position_partial_done[sym] = False
                self.position_scaleout_stage[sym] = 0
                removed_any = True
    ...
    async def on_signal(self, sig: Signal):
        ...
        # --- [DYNAMIC REGIME SWITCHER] ---
        try:
            from core.regime_manager import RegimeManager
            regime_flags = RegimeManager.get_dynamic_flags(sig.symbol)
        except Exception as e:
            logger.error(f"[REGIME ERROR] 국면 분석 실패 (기본값 사용): {e}")
            regime_flags = {"force_bluefrog": False, "margin_scale": 1.0}
            
        # [청개구리 모드 토글] USE_BLUEFROG=True: 역매매(롱->sell, 숏->buy) / False: 정매매(롱->buy, 숏->sell)
        base_bluefrog = getattr(CFG, "USE_BLUEFROG", True)
        is_bluefrog = base_bluefrog or regime_flags.get("force_bluefrog", False)

        mode_str = "역방향" if is_bluefrog else "순방향"
        # [2026-09-02] 방향 게이트를 **최종 방향** 기준으로 통일.
        # 종전에는 게이트가 else(정방향) 분기 안에만 있어, 역매매 모드에서는
        # 통째로 건너뛰어졌다. 바로 위 주석이 밝힌 의도와 반대로 동작한 것이다.
        # 8403~8410은 이미 '반전 후 방향에 게이트'로 되어 있어 그 형태에 맞춘다.
        side = ("sell" if sig.direction == "long" else "buy") if is_bluefrog \
            else ("buy" if sig.direction == "long" else "sell")
        if getattr(self.cfg, "USE_MARKET_GATE", False) and sig.direction in ("long", "short"):
            td = int(getattr(sig, "trend_dir", 0) or 0)
            final_long = (side == "buy")
            if td != 0 and (td > 0) != final_long:
                logger.info(f"[GATE BLOCK] {sig.symbol} 최종 "
                            f"{'LONG' if final_long else 'SHORT'}({mode_str}) — 추세 역행(trend={td})")
                return
        self.position_trade_modes[sig.symbol] = mode_str
        ...
        async with self.lock:
            ...
            if result:
                ...
                self.position_scaleout_stage[sig.symbol] = 0    # [2026-09-05] 다단 단계 초기화
                ...
                # [진입유실 근본수정] 역방향(BLUEFROG) 모드에서는 신호방향과 실제 체결방향이 반대다.
                #  sig.direction(신호)을 기록하면 진입행은 LONG 버킷, 청산행(buy=숏청산)은 SHORT 버킷으로
                #  갈라져 페어링이 영구 실패하고, 대시보드가 진입시각/진입가를 역산해 만들어낸다.
                #  거래소에 실제로 나간 방향(result["side"])을 기록해야 한다.
                executed_side = str(result.get("side") or "").strip().lower()
                if executed_side not in ("long", "short", "buy", "sell"):
                    executed_side = sig.direction
                    logger.warning(
                        f"[CSV LOG] {sig.symbol} 체결방향 확인 불가 → 신호방향({sig.direction})으로 기록"
                    )
                logger.info(
                    f"[ORDER] {sig.symbol} {executed_side.upper()} 실행 완료 "
                    f"(신호={sig.direction.upper()})"
                )
                ...
                csv_log({
                    ...
                    "trade_mode": self.position_trade_modes.get(sig.symbol, "역방향" if getattr(CFG, "USE_BLUEFROG", True) else "순방향"),
                })

                # 진입 성공 텔레그램 알림
                #  [치명버그 수정] place_order가 sl_price/tp_price를 문자열('0.34548')로 돌려주는 경우가 있어
                #  :.6f 포맷이 TypeError("Unknown format code 'f' for object of type 'str'")로 터졌다.
                #  이 예외는 scanner의 종목 루프 try에서 잡혀 경고 한 줄만 남기고 진입 후처리를 중단시킨다
                #  → 알림 유실뿐 아니라 이후 단계가 전부 건너뛰어진다. 반드시 float 변환 후 포맷한다.
                def _fmt_px(v):
                    try:
                        return f"{float(v):.6f}"
                    except (TypeError, ValueError):
                ...
                send_telegram_alert(
                    f"🟢 *[포지션 진입 완료]*\n"
                    f"종목: {sig.symbol}\n"
                    f"구분: {executed_side.upper()} 진입 (신호: {sig.direction.upper()})\n"
    ...
    async def _risk_check(self, sig: Signal) -> tuple[bool, str]:
        ...
        try:
            ...
            if ticker:
                ...
                # ── [2026-09-04] W3·W5 문턱을 설정값으로 뺀다 ──────────────────
                # 두 가드는 15분봉 기준으로 하드코딩돼 있었다. 일봉으로 바꾸자 즉시 걸렸다
                # (실측: ARB 시세 0.13125 vs 시그널 종가 0.12398 = 이격 5.5% > 5% 차단).
                # 일봉 신호는 **직전에 닫힌 일봉의 종가**를 기준가로 들고 오므로, 진입 시점의
                # 시세와 벌어지는 게 정상이다. 봉 길이가 길수록 이격도 커진다.
                # 기본값은 종전 그대로 두어 다른 타임프레임의 동작을 바꾸지 않는다.
                # W3. Volatility
                _vol_cap = float(getattr(self.cfg, "MAX_24H_VOLATILITY_PCT", 0.15))
                if high24 > 0 and low24 > 0 and _vol_cap > 0:
                    volatility_24h = (high24 - low24) / low24
                    if volatility_24h > _vol_cap:
                        return False, (f"극단적 변동성 서킷브레이커 발동 "
                                       f"(24h 고저차: {volatility_24h*100:.1f}% > {_vol_cap*100:.0f}%)")

                # W5. Fat-Finger
                _dev_cap = float(getattr(self.cfg, "MAX_ENTRY_PRICE_DEV_PCT", 0.05))
                if last > 0 and sig.close > 0 and _dev_cap > 0:
        ...
        if sig.strategy_type in ("DoubleBottom", "DoubleTop"):
            ...
            # MA 교차는 교차 발생 자체가 신호다. 문턱을 두면 검증 조건과 어긋난다.
            required_strength = 0
        elif sig.strategy_type == "Sniper15":
            # 신호 수식 5조건 + 두 라인 돌파가 이미 필터다. 8401·8409와 동일 처리.
            required_strength = 0
        else:
