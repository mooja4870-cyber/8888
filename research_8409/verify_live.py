"""라이브 8409 코드를 그대로 import해 검증 결과를 재현하는지 확인한다."""
...
class LiveSig:
    ...
    def at(self, df, i):
        w=df.iloc[max(0,i-80):i+1]
        if len(w)<55: return None
...
SEED=10.0
print("LOOKBACK=%s  SL_MULT=%s  TF=%s"%(LIVECFG.TSMOM_LOOKBACK_BARS,LIVECFG.TSMOM_SL_ATR_MULT,LIVECFG.TIMEFRAME))
for tag,sig in (("라이브 core/strategy.py",LiveSig()),("참조 Donchian(20)",Donchian(20))):
    tr=sorted(simulate_portfolio(data,sig,30,max_positions=4,notional=7.0),key=lambda x:x["t"])
