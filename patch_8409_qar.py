...
def create_strategy_qar():
    qar_code = '''#!/usr/bin/env python3
"""
Quantum Adaptive Regime & Dynamic Volatility-Targeted Engine (QAR-ARE)
─────────────────────────────────────────────────────────────────────────────
8409 봇 (Binance USDT-M 선물) 전용 기관급 퀀트 알파 엔진
...
5. John F. Ehlers (2013/2020): Cycle Analytics for Traders
   - 2-Pole SuperSmoother Digital Signal Processing (DSP) 필터 (무지연 잡음 제거)
6. Crypto High-Frequency Literature / Cartea (2015):
   - Binance 실시간 펀딩비 쏠림 역이용 숏 스퀴즈 알파 부스터
"""
import numpy as np
import pandas as pd
import warnings
...
def generate_qar_signal(df: pd.DataFrame, 
    ...
    반환값:
      ...
      strength: 0.0 ~ 1.0
      info: 신호 설명
      metrics: {ssf, ker, atr, qar_score, sl_price, tp_price, close, regime}
    """
    if df is None or len(df) < 50:
        return "hold", 0.0, "미충분 데이터", {}

    feats_df = QARFeatureEngine.compute_features(df)
    ...
    # 펀딩비 숏 스퀴즈 부스터 (바이낸스 특화)
    # 펀딩비가 -0.02% 이하로 극단적 음수이면 숏 스퀴즈 롱 부스터(+0.08 점수 가산)
    ...
    if score >= score_threshold and c >= ema200 and ssf >= prev["ssf"]:
        ...
        info = f"[QAR-ARE] 순수추세(KER={ker:.2f}) + 롱 모멘텀(Score={score:.2f}) | TP:+{tp_pct:.1f}% SL:-{sl_pct:.1f}% (RR {tp_mult/sl_mult:.2f}:1)"
    ...
    # 숏 신호 조건: 점수 < -임계값, 종가 < 200 EMA 및 SSF 하락
        ...
        info = f"[QAR-ARE] 순수추세(KER={ker:.2f}) + 숏 모멘텀(Score={score:.2f}) | TP:+{tp_pct:.1f}% SL:-{sl_pct:.1f}% (RR {tp_mult/sl_mult:.2f}:1)"
        return direction, strength, info, metrics
...
'''
    ...
    with open(target_path, "w", encoding="utf-8") as f:
        f.write(qar_code)
    print(f"✅ strategy_qar.py 생성 완료: {target_path}")
...
def patch_strategy_py():
    ...
    # Import 추가
    if "from core.strategy_qar import generate_qar_signal" not in code:
        code = "from core.strategy_qar import generate_qar_signal\n" + code

    # QAR 시그널 생성 함수
    qar_func = '''
# ═══════════════════════════════════════════════════════════════════════════════
# [2026-08-26] 전략 전면 혁신 — QAR-ARE (Quantum Adaptive Regime & Volatility Engine)
#
# 저명 퀀트 문헌 융합:
#   - Ehlers 2-Pole SuperSmoother (DSP 디지털 신호처리 무지연 필터)
#   - Kaufman Efficiency Ratio (KER 국면 판별, 횡보장 휩쏘 원천 차단)
#   - Marcos López de Prado Triple Barrier Method (동적 2.5/1.2 ATR TP/SL)
#   - Moskowitz & Harvey Volatility Scaling (변동성 역비례 사이징)
#   - Binance Funding Rate Squeeze Alpha Booster
# ═══════════════════════════════════════════════════════════════════════════════


def _generate_signal_qar(self, df: pd.DataFrame, symbol: str, **kwargs) -> Signal:
    """QAR-ARE 시그널 생성기 (Ehlers SSF + Kaufman ER + Triple Barrier)."""
    if df is None or len(df) < 50:
        return Signal(
    ...
    try:
        ...
        direction, strength, info, metrics = generate_qar_signal(
            df, ker_threshold=ker_th, score_threshold=score_th, tp_mult=tp_mult, sl_mult=sl_mult, funding_rate=funding_rate
        )
...
'''
    if "StrategyEngine.generate_signal = _generate_signal_qar" not in code:
        code += qar_func

    with open(strat_path, "w", encoding="utf-8") as f:
        f.write(code)
    print("✅ core/strategy.py QAR-ARE 바인딩 완료")
...
def patch_trader_py():
    ...
    # 1. 신호 강도 게이트 추가
    if 'elif sig.strategy_type == "QAR-ARE":' not in code:
        target = 'elif sig.strategy_type == "DualBB":'
        replacement = '''elif sig.strategy_type == "QAR-ARE":
    ...
    with open(trader_path, "w", encoding="utf-8") as f:
        f.write(code)
    print("✅ core/trader.py QAR-ARE 게이트 및 Triple Barrier 반영 완료")
...
def patch_config_py():
    ...
    if "QAR_KER_PERIOD" not in code:
        target = "    USE_BLUEFROG: bool ="
        qar_fields = '''    # ── QAR-ARE 전략 설정 ──────────────────────────────────
    QAR_KER_PERIOD: int = 20
    QAR_KER_THRESHOLD: float = 0.35
    QAR_SSF_PERIOD: int = 10
    QAR_SCORE_THRESHOLD: float = 0.25
    QAR_TP_MULT: float = 2.5
    QAR_SL_MULT: float = 1.2
    QAR_MAX_HOLDING_HOURS: float = 48.0
    
'''
        code = code.replace(target, qar_fields + target)
        with open(cfg_path, "w", encoding="utf-8") as f:
            f.write(code)
        print("✅ core/config.py QAR-ARE 필드 추가 완료")
    else:
        print("ℹ️ core/config.py QAR-ARE 필드 이미 존재")
...
def patch_config_json():
    ...
    data["STRATEGY_TYPE"] = "QAR-ARE (Ehlers DSP + Kaufman ER + Triple Barrier)"
    data["TIMEFRAME"] = "15m"
    data["QAR_KER_PERIOD"] = 20
    data["QAR_KER_THRESHOLD"] = 0.35
    data["QAR_SSF_PERIOD"] = 10
    data["QAR_SCORE_THRESHOLD"] = 0.25
    data["QAR_TP_MULT"] = 2.5
    data["QAR_SL_MULT"] = 1.2
    data["MAX_HOLDING_HOURS"] = 48.0
    data["ALLOW_LONG"] = True
    data["ALLOW_SHORT"] = True
    data["LEVERAGE"] = 5
    data["MARGIN_USDT"] = 1.5
    data["USE_DYNAMIC_SLTP"] = True

    with open(cfg_json_path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    print("✅ config.json QAR-ARE 파라미터 및 15m 타임프레임 갱신 완료")
...
### 변경 내용
* **QAR-ARE (Quantum Adaptive Regime & Dynamic Volatility-Targeted Engine) 전면 도입**
  - 금융공학 석학 및 저명 퀀트 문헌 융합 기반 혁신 알고리즘 엔진 탑재:
    1. **Ehlers 2-Pole SuperSmoother (DSP)**: 위상 지연 없는 고주파 노이즈 완벽 필터링 (`core/strategy_qar.py`)
...
def git_commit_8409():
    cmd = (
        f"cd {BOT_DIR} && "
        "git add . && "
        "git commit -m 'feat: 8409 QAR-ARE 퀀트 전략 전면 도입 및 15m 타임프레임 고도화 v10.0.0' && "
        "git tag v10.0.0 && "
...
if __name__ == "__main__":
    ...
    update_ver_md()
    git_commit_8409()
    print("🚀 8409 QAR-ARE 패치 및 깃 반영 완료!")
